...
function narrAudio(){if(NV.a)return NV.a;const a=NV.a=new Audio();a.preload='auto';
  ...
  a.addEventListener('pause',()=>{if(!a.ended&&NV.st!=='err'&&NV.key)set('pause')});
  a.addEventListener('ended',()=>{set('done');const i=narrEl()&&narrEl().querySelector('.nprog i');if(i)i.style.width='100%';const f=NV.onEnd;NV.onEnd=null;if(f)f()});
  a.addEventListener('error',()=>{if(NV.key&&a.getAttribute('src'))set('err')});
  a.addEventListener('timeupdate',()=>{const el=narrEl();if(!el||!a.duration)return;const i=el.querySelector('.nprog i');if(i)i.style.width=(a.currentTime/a.duration*100)+'%'});
  return a}
function narrStop(){try{window.speechSynthesis&&speechSynthesis.cancel()}catch(e){}if(NV.a){try{NV.a.pause()}catch(e){}}NV.key=null;NV.st='idle';NV.onEnd=null;narrUI()}
function narrPlay(key,o){o=o||{};const a=narrAudio();try{a.pause()}catch(e){}
  NV.key=key;NV.onEnd=o.onEnd||null;NV.st='load';narrUI();
  a.src=AUD+key+'.mp3';a.defaultPlaybackRate=a.playbackRate=RATES[S.rate][0];
...
/* صدای تشویق کوتاه */
const VFX={};let vfxT=0;
function vfx(k,delay){if(S.mute)return;const now=Date.now();if(now-vfxT<1500)return;vfxT=now;setTimeout(()=>{try{const a=VFX[k]||(VFX[k]=new Audio(AUD+k+'.mp3'));a.currentTime=0;a.playbackRate=1;const p=a.play();if(p&&p.catch)p.catch(()=>{})}catch(e){}},delay||0)}
function vfxPreload(){['ok1','ok2','ok3','ok4','ok5','no1','no2'].forEach(k=>{if(!VFX[k]){try{const a=new Audio();a.preload='auto';a.src=AUD+k+'.mp3';VFX[k]=a}catch(e){}}})}
/* خواندن متن‌های پویا با صدای سیستم (فقط اگه دستگاه صدای فارسی داشته باشه) */
let faV=null;
...
function go(fn){const run=()=>{closeSheet(true);narrStop();fn();window.scrollTo(0,0)};if(document.startViewTransition&&!RM){try{document.startViewTransition(run);return}catch(e){}}run()}
...
function finish(){
  ...
  if(st||mode!=='stage'){setTimeout(sWin,300);if(st)setTimeout(confetti,350);vfxT=0;vfx(st?'win':'ok2',900)}
}
